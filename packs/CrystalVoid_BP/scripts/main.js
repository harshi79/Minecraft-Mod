...
const REALM_ARRIVAL = { x: 0.5, y: REALM_FLOOR_Y + 1, z: 5.5 };
const REALM_CENTER = { x: 0.5, y: REALM_FLOOR_Y + 1, z: 0.5 };
const MARKER_LOCATION = { x: 0, y: 60, z: 0 };
...
async function useVoidAnchor(player) {
...
async function returnFromRealm(player) {
    const destination = getReturnPoint(player);
    player.onScreenDisplay.setActionBar("§dThe anchor remembers your world…");
    await teleportWithLoadedArea(player, destination.dimension, destination, { x: destination.x, y: destination.y, z: destination.z + 1 });
    protectArrival(player);
...
function getReturnPoint(player) {
...
async function teleportWithLoadedArea(player, dimension, destination, facingLocation) {
...
function ensureRealmBuilt() {
    if (!realmBuildPromise) {
        realmBuildPromise = buildRealmIfNeeded().catch((error) => {
...
async function buildRealmIfNeeded() {
    const realm = world.getDimension(REALM_ID);
...
function* createRealmBuildSteps(realm) {
    ...
    for (let depth = 0; depth < radii.length; depth++) {
        ...
        for (let z = -radius; z <= radius; z++) {
            ...
            realm.fillBlocks(new BlockVolume({ x: -halfWidth, y, z }, { x: halfWidth, y, z }), layerBlock);
            operations++;
            if (operations % 18 === 0) {
                yield;
            }
...
function setBlock(dimension, location, permutation) {
...
function protectArrival(player) {
    try {
...
function playPortalSound(player, pitch) {
    try {
